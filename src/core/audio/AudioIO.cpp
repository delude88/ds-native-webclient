...
AudioIO::AudioIO(DigitalStage::Api::Client &client)
    : client_(client), num_devices_(0), watching_device_updates_(false) {
  attachHandlers();
}
    ...
    if (local_device) {
      ...
      if (local_device->audioDriver && !local_device->audioDriver->empty()) {
        ...
        if (local_device->inputSoundCardId) {
          ...
          if (sound_card) {
            setInputSoundCard(*sound_card, local_device->sendAudio, client_);
          } else {
    ...
    if (store->isReady() && audio_driver && !audio_driver->empty()) {
      ...
      if (local_device) {
        ...
        if (input_sound_card && input_sound_card->online) {
          setInputSoundCard(*input_sound_card, local_device->sendAudio, client_);
        }
