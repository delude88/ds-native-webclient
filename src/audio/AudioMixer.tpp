...
template<class T>
AudioMixer<T>::AudioMixer(DigitalStage::Api::Client &client) {
...
template<class T>
void AudioMixer<T>::attachHandlers(DigitalStage::Api::Client &client) {
  ...
  client.audioTrackRemoved.connect([this](const AudioTrack &audio_track, const DigitalStage::Api::Store *) {
    volume_map_.erase(audio_track._id);
  });
  client.customAudioTrackVolumeAdded.connect([this](const CustomAudioTrackVolume &custom_volume,
                                                    const DigitalStage::Api::Store *store) {
  ...
  client.customAudioTrackVolumeChanged.connect([this](const std::string &id, const nlohmann::json &update,
                                                      const DigitalStage::Api::Store *store) {
    if (update.contains("volume") || update.contains("muted")) {
  ...
  client.customAudioTrackVolumeRemoved.connect([this](const CustomAudioTrackVolume &custom_volume,
                                                      const DigitalStage::Api::Store *store) {
    auto audio_track = store->audioTracks.get(custom_volume.audioTrackId);
    assert(audio_track);
    volume_map_[audio_track->_id] = calculateVolume(*audio_track, *store);
  ...
  client.customStageDeviceVolumeAdded.connect([this](const CustomStageDeviceVolume &custom_volume,
                                                     const DigitalStage::Api::Store *store) {
    for (const auto &audio_track: store->audioTracks.getAll()) {
  ...
  client.customStageDeviceVolumeChanged.connect([this](const std::string &custom_stage_device_volume_id,
  ...
  client.customStageDeviceVolumeRemoved.connect([this](const CustomStageDeviceVolume &custom_volume,
                                                       const DigitalStage::Api::Store *store) {
    for (const auto &audio_track: store->audioTracks.getAll()) {
      if (audio_track.stageDeviceId == custom_volume.stageDeviceId) {
        volume_map_[audio_track._id] = calculateVolume(audio_track, *store);
  ...
  client.customStageMemberVolumeAdded.connect([this](const CustomStageMemberVolume &custom_volume,
                                                     const DigitalStage::Api::Store *store) {
    for (const auto &audio_track: store->audioTracks.getAll()) {
  ...
  client.customStageMemberVolumeChanged.connect([this](const std::string &custom_stage_member_volume_id,
                                                       const nlohmann::json &update,
                                                       const DigitalStage::Api::Store *store) {
    if (update.contains("volume") || update.contains("muted")) {
  ...
  client.customStageMemberVolumeRemoved.connect([this](const CustomStageMemberVolume &custom_volume,
                                                       const DigitalStage::Api::Store *store) {
    for (const auto &audio_track: store->audioTracks.getAll()) {
      if (audio_track.stageMemberId == custom_volume.stageMemberId) {
        volume_map_[audio_track._id] = calculateVolume(audio_track, *store);
  ...
  client.customGroupVolumeAdded.connect([this](const CustomGroupVolume &custom_volume,
                                               const DigitalStage::Api::Store *store) {
  ...
  client.customGroupVolumeChanged.connect([this](const std::string &custom_group_volume_id,
                                                 const nlohmann::json &update,
                                                 const DigitalStage::Api::Store *store) {
    if (update.contains("volume") || update.contains("muted")) {
  ...
  client.customGroupVolumeRemoved.connect([this](const CustomGroupVolume &custom_volume,
                                                 const DigitalStage::Api::Store *store) {
    for (const auto &stage_member: store->getStageMembersByGroup(custom_volume.groupId)) {
      for (const auto &audio_track: store->audioTracks.getAll()) {
        if (audio_track.stageMemberId == stage_member._id) {
...
std::optional<std::pair<T,
                        bool>> AudioMixer<T>::getGain(const std::string &audio_track_id) const {
  if (volume_map_.count(audio_track_id)) {
